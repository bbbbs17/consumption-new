...
import com.example.ImproveComsumption.dto.PasswordResetRequestDto;
import com.example.ImproveComsumption.repository.MemberRepository;
import com.example.ImproveComsumption.repository.PasswordResetTokenRepository;
...
public class PasswordResetService {
    ...
    public void sendResetCode(String email) {
        ...
        // 2) (선택) 이미 OAuth2 회원이면, 비밀번호 재설정 불가하도록 처리할 수 있음
         if (member.getOAuthType() != OAuthType.NONE) {
        ...
        PasswordResetToken resetToken = new PasswordResetToken(member, code, expiresAt);
        tokenRepository.save(resetToken);

        // 6) 이메일 전송
        String subject = "[소비분석캘린더] 비밀번호 재설정을 위한 인증번호 안내";
    ...
    private PasswordResetToken getTokenByEmail(String email) {
        Optional<PasswordResetToken> optionalToken = tokenRepository.findByMember_Email(email);
